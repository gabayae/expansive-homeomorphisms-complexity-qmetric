...
% ── Repository URL ──
\newcommand{\repourl}{https://github.com/guydev42/expansive-homeomorphisms-complexity-qmetric}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\itshape Expansive homeomorphisms on complexity quasi-metric spaces}
...
{\huge\bfseries Expansive homeomorphisms on\\[0.5em]
 complexity quasi-metric spaces}

\vspace{0.4cm}
{\Large\itshape A bridge between dynamical systems\\[0.2em]
 and computational complexity theory}

\vspace{1.0cm}

% TODO: Replace placeholder author names and affiliations before submission
{\large
\textbf{First Author}$^{\dagger}$\quad and\quad
\textbf{Second Author}$^{\ddagger}$
}

\vspace{0.6cm}

{\small
$^{\dagger}$\,Department of Mathematics, University One\\
Address Line, City, Country\\
\texttt{first.author@university-one.edu}

\vspace{0.4cm}

$^{\ddagger}$\,Department of Computer Science, University Two\\
Address Line, City, Country\\
\texttt{second.author@university-two.edu}
}
...
\[
  d_\C(f,g)
  \;=\; \sum_{n=1}^{\infty} 2^{-n}\,
    \max\!\Bigl\{0,\;\frac{1}{g(n)}-\frac{1}{f(n)}\Bigr\}.
\]
\end{definition}
...
\[
  \max\Bigl\{0,\frac{1}{h(n)}-\frac{1}{f(n)}\Bigr\}
  \;\le\;
  \max\Bigl\{0,\frac{1}{g(n)}-\frac{1}{f(n)}\Bigr\}
  +\max\Bigl\{0,\frac{1}{h(n)}-\frac{1}{g(n)}\Bigr\},
\]
...
\textbf{(iii)}  Each term $2^{-n}\max\{0,1/g(n)-1/f(n)\}\le
...
\[
  d_\C(g,f)
  = \sum_{n=1}^{\infty}2^{-n}\max\Bigl\{0,\frac{1}{n}-\frac{1}{n^2}\Bigr\}
...
\[
  d_\C^t(f,g) \;=\; d_\C(g,f)
  \;=\; \sum_{n=1}^{\infty}2^{-n}\max\Bigl\{0,\frac{1}{f(n)}-\frac{1}{g(n)}\Bigr\}.
\]
...
\[
  ...
  = \sum_{n=1}^\infty 2^{-n}
    \max\Bigl\{0,\frac{1}{g(n)+kc}-\frac{1}{f(n)+kc}\Bigr\}.
\]
...
\begin{align*}
  d_\C(\psi_\alpha(f),\psi_\alpha(g))
  &= \sum_{n=1}^{\infty}2^{-n}\max\Bigl\{0,
     \frac{1}{\alpha g(n)}-\frac{1}{\alpha f(n)}\Bigr\}\\
...
=\sum_{n=1}^\infty 2^{-n}\max\{0,\frac{1}{3n}-\frac{1}{3n^2}\}
...
The classical theory of hyperbolic dynamics, developed by
Bowen~\cite{bowen1975} and extended by Reddy~\cite{reddy1983},
shows that expansive homeomorphisms with
``canonical coordinates'' exhibit exponential contraction along
stable manifolds and exponential expansion along unstable
manifolds.  We now establish the precise form of this
hyperbolicity for the scaling transformation.
...
The authors thank \textit{[acknowledgements to be added]}.
