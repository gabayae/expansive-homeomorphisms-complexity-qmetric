...
The $\delta$-stable sets of this dynamics turn out to coincide
with asymptotic complexity classes, giving a dynamical
characterisation of objects familiar from complexity theory.
We then show that $\psi_\alpha$ contracts $d_\C^s$-distances
exactly by factor $1/\alpha$ per iterate, and we connect orbit
separation to the classical time hierarchy theorem of Hartmanis
and Stearns.  Unstable sets, conjugate dynamics, and a
no-compact-invariant-set theorem (showing that the standard
Bowen entropy is trivially zero on compact invariant sets) are
also worked out.  Concrete algorithms and Python
...
\[
  ...
  \;=\; 1.
\]
This saturates the bound $d_\C(g,f)\le w_\C(g)=1$ on the subspace
$\C_{\ge 1}$, and witnesses the asymmetry: the cost of swapping a
faster function for a slower one is bounded by the weight of the
target, with equality possible.
\end{example}
...
\begin{proof}
The inverse identity $\psi_\alpha\circ\psi_{1/\alpha}=\mathrm{id}$
is immediate.  By Lemma~\ref{lem:scaling-lip} applied to both
$d_\C$ and $d_\C^t$,
...
\textbf{Case 2: $\alpha>1$.}  Take any $\delta\in(0,1)$.  Let
$f\ne g$ in $\C$.  Since $(\C,d_\C^s)$ is a $T_0$ quasi-metric
space, at least one of $d_\C(f,g)$ or $d_\C(g,f)$ is positive.

If $d_\C(f,g)>0$, apply Corollary~\ref{cor:backward-expansion} with
the pair $(f,g)$: for some $k\ge 0$,
$d_\C(\psi_\alpha^{-k}(f),\psi_\alpha^{-k}(g))=\alpha^k d_\C(f,g)>\delta$,
and Definition~\ref{def:expansive} is satisfied with $n=-k$.

If instead $d_\C(f,g)=0$ but $d_\C(g,f)>0$, apply the same corollary
to the pair $(g,f)$: for some $k\ge 0$,
...
\end{enumerate}
Proposition~\ref{prop:no-compact-invariant} below shows that any
compact $\psi_\alpha$-invariant subset of $(\C,d_\C^s)$ is
$d_\C^s$-trivial, so the entropy of $\psi_\alpha|_K$ is always~$0$
for such~$K$.
\end{remark}
...
$\psi_\alpha$-invariant, then the diameter
$\mathrm{diam}(K)=\sup_{f,g\in K}d_\C^s(f,g)$ satisfies
$\mathrm{diam}(\psi_\alpha(K))=(1/\alpha)\mathrm{diam}(K)$.
But $\psi_\alpha(K)=K$, so $\mathrm{diam}(K)=0$.
\end{proof}
...
\begin{proposition}[Finite invariant sets have zero entropy]\label{prop:entropy-upper}
For the scaling map $\psi_\alpha$ on a finite
$\psi_\alpha$-invariant set $K=\{f_1,\ldots,f_m\}\subset\C$, the
topological entropy satisfies $h(\psi_\alpha|_K)=0$.
\end{proposition}

\begin{proof}
A finite set is a special case of a compact invariant set.  By
Proposition~\ref{prop:no-compact-invariant}, $d_\C^s(f,g)=0$ for
all $f,g\in K$.  Thus $K$ is a single $d_\C^s$-equivalence class,
the only $(n,\eps)$-spanning set is $E=K$ itself (with
$r(n,\eps)=1$ for all $n,\eps>0$), and
$h(\psi_\alpha|_K)=\lim_{n\to\infty}\frac{1}{n}\log 1=0$.
\end{proof}
