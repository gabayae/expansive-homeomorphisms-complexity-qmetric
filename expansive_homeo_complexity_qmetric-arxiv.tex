...
Throughout, $\N=\{1,2,3,\dots\}$.  Let $\C$ denote the set of all
functions
\[
  f\colon\N\to[1,\infty)\quad\text{with}\quad
  \sum_{n=1}^\infty 2^{-n}/f(n)<\infty.
\]
Each such function represents the running-time profile of an
algorithm: $f(n)$ is the time required on inputs of size~$n$.

\begin{remark}[Why $f(n)\ge 1$]\label{rem:lower-bound}
The lower bound $f(n)\ge 1$ encodes the elementary fact that any
computation on an input of size~$n$ takes at least one elementary
step.  This is the standard convention in the Schellekens
framework~\cite{schellekens1995,romaguera1999}: without it, the
quasi-metric $d_\C$ is not bounded --- and in fact need not even take
finite values --- on the larger class of strictly positive functions.
The summability requirement
$\sum 2^{-n}/f(n)<\infty$ is automatic when $f$ is bounded below
by a positive constant, and ensures that $d_\C$ is everywhere finite.
\end{remark}
...
\begin{example}[The bound $d_\C\le 1$ is sharp]\label{ex:counter-bounded}
The bound in Theorem~\ref{thm:dc-props}(iii) cannot be improved.  Take
$g(n)=1$ for all $n$ and, for each $k\in\N$, the constant function
$f_k(n)=k$.  Both lie in~$\C$.  Since $g\le f_k$ pointwise, we have
$d_\C(g,f_k)=0$.  In the reverse direction,
\[
  d_\C(f_k,g)
  \;=\;\sum_{n=1}^\infty 2^{-n}\,\Bigl(1-\tfrac{1}{k}\Bigr)
  \;=\; 1-\tfrac{1}{k},
\]
which approaches $1$ as $k\to\infty$.  Thus $\sup_{f,g\in\C}d_\C(f,g)=1$,
but the value $1$ itself is not attained on $\C$.
\end{example}
