...
Here is a preview of the main results; the reader may want to come
back to this summary while working through the technical sections.

The first result characterises exactly when the scaling
transformation is expansive.
...
The second result gives the stable sets of this dynamics a clean
...
\begin{keyresult}[Corollary~\ref{cor:exact-contraction}: Exact contraction rate]
The scaling map $\psi_\alpha$ ($\alpha>0$) contracts distances
exactly by factor $1/\alpha$ per forward iterate: the contraction
rate is $\lambda=1/\alpha$ and the constant is $C=1$.
\end{keyresult}

The fourth result connects the dynamical picture to a classical
...
Each of these results is proved in detail, with an accompanying
algorithm and a Python implementation; all code lives in the
